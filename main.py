...
def preprocessing(data_path="SMSSpamCollection"):
    ...
    print(
        f"스팸이 아닌 것 : {(df.label == 0).sum()}, 스팸인 것 : {(df.label == 1).sum()}"
    )
...
def train(
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 2 == 0:
            print(
                f"[{model_name}] Epoch {epoch+1:2d}/{num_epochs} | "
...
MODEL_MAPS = {"rnn": SpamRNN, "lstm": SpamLSTM, "gru": SpamGRU}
...
if __name__ == "__main__":
    ...
    for name in ["rnn", "lstm", "gru"]:
        ...
        model = MODEL_MAPS[name](vocab_size=vocab_size)
