...
from models.rnn import SpamLSTM, SpamRNN, SpamGRU
from utils.visualize import plot_confusion_matrices, plot_comparison
...
if __name__ == '__main__':
    ...
    #훈련, 평가
    o_history = main.train(original_lstm, original_train, original_valid,
                                    criterion, optimizer, num_epochs=30,
                                    device=device, model_name='original')

    o_labels, o_preds = main.evaluate(original_lstm, original_test, device, model_name='original')
    ...
    #허깅페이스에서 다운로드받은 토크나이저의 트것ㅇ 속에 vocab_size가 미리 정의되어 있음
    hug_lstm = SpamLSTM(tokenizer.vocab_size)
    new_optimizer = optim.Adam(hug_lstm.parameters())
    h_history = main.train(hug_lstm, hug_train, hug_valid,
                            criterion, new_optimizer, num_epochs=30,
                            device=device, model_name='huggingface')
    h_labels, h_preds = main.evaluate(hug_lstm, hug_test, device, model_name='hugging')
